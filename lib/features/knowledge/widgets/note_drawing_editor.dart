...
class _NoteDrawingEditorState extends State<NoteDrawingEditor> {
  ...
  void _update(Offset local, Size size) {
    ...
    final points = [...stroke.points, _point(local, size)];
    _active = NoteDrawingStroke(tool: stroke.tool, color: stroke.color, width: stroke.width, opacity: stroke.opacity, points: points);
    setState(() {});
  }
  ...
  NoteDrawingPoint _point(Offset p, Size size) => NoteDrawingPoint(
        size.width == 0 ? 0 : (p.dx / size.width).clamp(0, 1),
        size.height == 0 ? 0 : (p.dy / size.height).clamp(0, 1),
      );
  ...
  void _eraseAt(Offset local, Size size) {
    ...
    if (changed) setState(() => _strokes = kept);
  }
  ...
  void _redoStroke() {
    ...
    setState(() { _strokes = [..._strokes, _redo.removeLast()]; });
  }
  ...
  Widget build(BuildContext context) {
    final tools = <NoteDrawingTool>[NoteDrawingTool.pen, NoteDrawingTool.highlighter, NoteDrawingTool.line, NoteDrawingTool.rectangle, NoteDrawingTool.ellipse, NoteDrawingTool.polygon, NoteDrawingTool.eraser];
    return Scaffold(
      appBar: AppBar(
        title: const Text('手绘笔记'),
        actions: [
        ...
        Expanded(
          child: LayoutBuilder(builder: (context, constraints) {
            ...
            return GestureDetector(
              ...
              onPanEnd: (_) => _end(),
              child: CustomPaint(painter: _DrawingPainter([..._strokes, if (_active != null) _active!]), size: size),
            );
          }),
        ),
  ...
  String _label(NoteDrawingTool tool) => switch (tool) {
        NoteDrawingTool.pen => '笔', NoteDrawingTool.highlighter => '荧光笔', NoteDrawingTool.line => '直线', NoteDrawingTool.rectangle => '矩形', NoteDrawingTool.ellipse => '椭圆', NoteDrawingTool.polygon => '多边形', NoteDrawingTool.eraser => '橡皮擦',
      };
}
...
class _DrawingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final stroke in strokes) {
      ...
      } else if (stroke.tool == NoteDrawingTool.polygon) {
