          ...
          Padding(
            ...
            child: Row(children: [
              ...
              DropdownButton<ReaderNoteFormat>(
                value: _noteFormat,
                onChanged: _previewMode || _recording ? null : (value) { if (value != null) setState(() => _noteFormat = value); },
                items: const [
                  DropdownMenuItem(value: ReaderNoteFormat.markdown, child: Text('Markdown')),
                  DropdownMenuItem(value: ReaderNoteFormat.markdownHtml, child: Text('Markdown-HTML')),
                ],
              ),
            ]),
          ),
          ...
          if (!_previewMode && !_detached)
            SafeArea(
              ...
              child: Padding(
                ...
                child: Row(children: [
                  OutlinedButton.icon(onPressed: _saving || _recording ? null : _takePhoto, icon: const Icon(Icons.photo_camera_outlined), label: const Text('拍照')),
                  const SizedBox(width: 8),
                  OutlinedButton.icon(onPressed: _saving ? null : _toggleRecording, icon: Icon(_recording ? Icons.stop_circle_outlined : Icons.mic_none_outlined), label: Text(_recording ? '停止录音' : '录音')),
...
class _CameraCapturePageState extends State<_CameraCapturePage> {
  ...
  Future<void> _capture() async {
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('拍照失败：$error')));
    }
  }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: FutureBuilder<void>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) return Center(child: Text('摄像头初始化失败：${snapshot.error}'));
            return Stack(alignment: Alignment.bottomCenter, children: [Positioned.fill(child: CameraPreview(_controller)), Padding(padding: const EdgeInsets.all(24), child: FloatingActionButton(onPressed: _capture, child: const Icon(Icons.camera_alt))) ]);
          },
        ),
      );
