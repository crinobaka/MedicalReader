      ...
      children: [
        _section(context, '阅读器外观', '先选整体气质，再按习惯微调', [
          ...
          _selectRow(context, Icons.view_agenda_outlined, '页面布局', _pageLayoutLabel(options.pageLayout), () async {
            final value = await _choose(context, '页面布局', options.pageLayout, const {'one': '单页', 'two': '双页（左右并排）', 'three': '三页（连续并排）'});
        ...
        _section(context, 'DIY', '普通用户可以忽略；高级用户保留自由度', [
          ExpansionTile(
            ...
            subtitle: Text('主题 ${options.themePreset} · ${_pageLayoutLabel(options.pageLayout)}'),
            children: [
              Align(
