...
class ReaderSettingsPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final children = [
      _section(context, '阅读器外观', '先选整体气质，再按习惯微调', [
        _ThemePresetSelector(
          ...
          onChanged: (value) =>
              onChanged(options.copyWith(themePreset: value)),
        ),
        ...
        _selectRow(
          ...
          () async {
            ...
            if (value != null) {
              onChanged(options.copyWith(pageLayout: value));
            }
          },
        ),
        _selectRow(
      ...
      _section(context, '阅读时显示什么', '只留下真正需要的控件，减少长时间阅读的视觉噪音', [
        _switch(
          ...
          (v) => onChanged(options.copyWith(showLocationBar: v)),
        ),
        _switch(
          context,
        ...
        _switch(
          ...
          (v) => onChanged(options.copyWith(showBookTreeButton: v)),
        ),
        _switch(
          context,
          ...
          (v) => onChanged(options.copyWith(showSearchButton: v)),
        ),
        _switch(
          context,
          ...
          (v) => onChanged(options.copyWith(showPageJumpButton: v)),
        ),
        _switch(
          context,
          ...
          (v) => onChanged(options.copyWith(showPageControls: v)),
        ),
      ]),
      _section(context, '页面裁剪', '设置这里只负责入口和显示控制', [
        _switch(
          ...
          '显示裁剪控制',
          null,
          options.showCropMargins,
          (v) => onChanged(options.copyWith(showCropMargins: v)),
        ),
        _selectRow(
          context,
          ...
          '裁剪模板',
          '打开当前页可视化编辑器',
          () => _editCropConfiguration(context),
        ),
      ]),
    ...
    final listView = ListView(
      ...
      shrinkWrap: scrollController == null ? false : false,
    ...
    // SettingsPage embeds this panel inside its own scrolling page. A
    // ListView with an unbounded height is illegal there and can result in a
    // blank page after the layout assertion. When height is unbounded, render
    // the same settings as a non-scrolling shrink-wrapped list and let the
    // parent own scrolling. When used by a modal Reader settings sheet the
    // panel keeps its original scrolling behavior.
    return LayoutBuilder(
      builder: (context, constraints) {
        if (constraints.hasBoundedHeight || scrollController != null) {
          return listView;
        }
        return ListView(
          shrinkWrap: true,
  ...
  String _toolbarLabel(String value) =>
      const {'auto': '自动（推荐）', 'top': '顶部', 'bottom': '底部'}[value] ??
      '自动（推荐）';

  String _canvasLabel(String value) =>
      const {
        'inherit': '跟随系统',
        'paper': '纸张',
        'dark': '暗色',
  ...
  String _pageLayoutLabel(String value) =>
      const {
        'one': '单页',
        'two': '双页（左右并排）',
        'three': '三页（连续并排）',
...
class _ThemePresetSelector extends StatelessWidget {
  ...
  static const presets = [
    (
      id: 'google',
      name: 'Google',
      description: 'Material：清晰、亲和、强调层级',
      icon: Icons.auto_awesome_rounded,
    ),
    (
      id: 'apple',
      name: 'Apple',
      description: '轻量、留白、圆润、少干扰',
      icon: Icons.phone_iphone_rounded,
    ),
    (
      id: 'github',
      name: 'GitHub',
      description: '紧凑、直接、适合长时间桌面使用',
...
class _ThemePresetCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final preview = ReaderUiTheme.resolve(
      preset.id,
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              Radio<String>(
                value: preset.id,
                groupValue: selected ? preset.id : null,
                onChanged: (_) => onTap(),
              ),
            ],
          ),
