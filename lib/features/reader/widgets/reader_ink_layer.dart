...
class _ReaderInkLayerState extends State<ReaderInkLayer>{
  ...
  List<Offset> _encode(List<Offset> p){final m=<Offset>[const Offset(.999999,.999999),Offset(_tool.index/10,.5),Offset(widget.color.value/0xffffffff,.5),Offset(widget.width/100,.5),Offset(_tool==ReaderInkTool.highlighter?.28:1,.5)];return [...m.map((x)=>Offset(x.dx*_size.width,x.dy*_size.height)),...p];}
