...
class PageBlockController extends ChangeNotifier {
  PageBlockController({required this.docId, required int pageCount, PageBlockManager? manager}) : pageCount = pageCount < 1 ? 1 : pageCount, manager = manager ?? PageBlockManager() {
  ...
  bool enabled = false, loading = false, adapting = false, editing = false;
  int currentPageIndex = 0, currentBlockIndex = 0;
  ...
  Future<void> enable({int? pageIndex}) async {
    ...
    notifyListeners();
    try { _setPosition(await navigation.first(docId, currentPageIndex)); unawaited(_prefetch()); } catch (e) { error = e; }
  ...
  Future<void> adaptCurrentPage({required ui.Image image, required double viewportWidth, required double viewportHeight}) async {
    ...
    adapting = true; notifyListeners();
    try {
      manager.configureLayout(pageIndex: page, image: image, viewportWidth: viewportWidth, viewportHeight: viewportHeight);
      ...
      if (oldBlock != null && oldBlock.source == PageBlockSource.manual) {
        ...
        currentBlockIndex = safeIndex; currentBlock = blocks[safeIndex];
      }
    } finally { adapting = false; notifyListeners(); }
  }
  Future<void> disable() async { enabled = false; loading = false; adapting = false; editing = false; currentBlock = null; manager.clear(); notifyListeners(); }
  Future<void> moveToPage(int pageIndex, {double? originalX, double? originalY}) async {
    if (!enabled) return;
    currentPageIndex = pageIndex.clamp(0, pageCount - 1).toInt(); loading = true; error = null; notifyListeners();
    try { _setPosition(originalX == null || originalY == null ? await navigation.first(docId, currentPageIndex) : await navigation.fromOriginalPosition(docId: docId, pageIndex: currentPageIndex, x: originalX, y: originalY)); unawaited(_prefetch()); }
    catch (e) { error = e; }
    finally { loading = false; notifyListeners(); }
  }
  Future<bool> next() async {
    ...
    try { final position = await navigation.next(docId: docId, pageIndex: currentPageIndex, blockIndex: currentBlockIndex, pageCount: pageCount); if (position == null) return false; _setPosition(position); unawaited(_prefetch()); return true; }
  ...
  Future<bool> previous() async {
    if (!enabled || loading || adapting || currentBlock == null) return false;
    loading = true; notifyListeners();
    try { final position = await navigation.previous(docId: docId, pageIndex: currentPageIndex, blockIndex: currentBlockIndex); if (position == null) return false; _setPosition(position); unawaited(_prefetch()); return true; }
    catch (e) { error = e; return false; }
    finally { loading = false; notifyListeners(); }
  }
  Future<void> saveManualBlocks(List<NormalizedRect> rects) async { await manager.saveManual(docId, currentPageIndex, rects); _setPosition(await navigation.first(docId, currentPageIndex)); notifyListeners(); }
  Future<void> saveOrderedBlocks(List<PageBlock> blocks) async {
    ...
    if (refreshed.isEmpty) { currentBlock = null; currentBlockIndex = 0; }
      ...
      if (previousRect != null) targetIndex = refreshed.indexWhere((b) { final r = b.rect; return r.x == previousRect.x && r.y == previousRect.y && r.width == previousRect.width && r.height == previousRect.height; });
      if (targetIndex < 0) targetIndex = currentBlockIndex;
      final safeIndex = targetIndex.clamp(0, refreshed.length - 1).toInt(); currentBlockIndex = safeIndex; currentBlock = refreshed[safeIndex];
    }
    notifyListeners();
  }
  Future<void> updateScrollPercent(double percent) async {
    final block = currentBlock; if (block == null) return;
    final next = block.copyWith(scrollPercent: percent); currentBlock = next;
