...
(function() {
  ...
  reader.buildPaginationMetrics = function() {
    const size = Math.max(1, this.pageSize()), maximum = Math.max(0, this.maxScroll());
    ...
    while ((node = walker.nextNode())) {
      ...
      for (const rect of range.getClientRects()) { if (rect.width <= 0 || rect.height <= 0) continue; const a = this.contentStart(rect), b = this.contentEnd(rect); start = start === null ? a : Math.min(start, a); end = end === null ? b : Math.max(end, b); }
      if (start !== null) { firstContent = firstContent === null ? start : Math.min(firstContent, start); lastContent = Math.max(lastContent, end || start); progressStops.push({scroll: Math.max(0, start), chars: totalChars}); }
      totalChars += chars;
    }
    for (const element of body.querySelectorAll('img, svg, image, video, canvas')) { const rect = element.getBoundingClientRect(); if (!rect.width || !rect.height) continue; firstContent = firstContent === null ? this.contentStart(rect) : Math.min(firstContent, this.contentStart(rect)); lastContent = Math.max(lastContent, this.contentEnd(rect)); }
    const minScroll = firstContent === null ? 0 : Math.min(maximum, this.alignToPage(firstContent));
    const endScroll = lastContent <= 0 ? 0 : this.alignToPage(lastContent - 1);
    const metrics = {minScroll: minScroll, maxScroll: Math.min(maximum, Math.max(minScroll, endScroll)), totalChars: Math.max(1, totalChars), progressStops: progressStops.sort((a, b) => a.scroll - b.scroll)};
