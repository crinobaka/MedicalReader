...
(function() {
  ...
  const maxScroll = function() { return Math.max(0, verticalContext() ? body.scrollHeight - window.innerHeight : body.scrollWidth - window.innerWidth); };
  reader.axis = function() { return verticalContext() ? 'y' : 'x'; };
  reader.position = position; reader.pageSize = pageSize; reader.maxScroll = maxScroll;
  reader.assignPagePosition = function(value) { const target = Math.min(Math.max(0, value), maxScroll()); if (verticalContext()) body.scrollTop = target; else body.scrollLeft = target; this.lockRootViewport(); this.lastPageScroll = target; return target; };
  reader.contentStart = function(rect) { const current = position(); return verticalContext() ? rect.top + current : rect.left + current; };
  reader.contentEnd = function(rect) { const current = position(); return verticalContext() ? rect.bottom + current : rect.right + current; };
  ...
  reader.paginate = function(direction) {
    ...
    this.setPagePosition(target); return 'scrolled';
  };
  ...
  const sanitizeLayout = function() {
    ...
    for (let i = 0; i < nodes.length; i++) { const el = nodes[i]; if (el === body || el.tagName === 'IMG' || el.tagName === 'SVG' || el.tagName === 'VIDEO' || el.tagName === 'CANVAS') continue; for (let j = 0; j < blocked.length; j++) el.style[blocked[j]] = ''; if (el.style.height && /^(100%|100vh|100vw)$/i.test(el.style.height)) el.style.height = ''; }
    const vertical = verticalContext(); const style = getComputedStyle(body);
    ...
    body.querySelectorAll('div, span').forEach(function(el) { const computed = getComputedStyle(el); if (computed.display !== 'inline-block' || !el.querySelector('p')) return; const rect = el.getBoundingClientRect(); const block = vertical ? rect.width : rect.height; const inline = vertical ? rect.height : rect.width; if (inline <= inlineExtent + 1 && block <= blockExtent + 1) return; el.style.setProperty('display', 'block', 'important'); if (!el.parentNode) return; el.parentNode.querySelectorAll('span:empty').forEach(function(strut) { if (strut.parentNode !== el.parentNode || isMeaningfulEmptySpan(strut) || getComputedStyle(strut).display !== 'inline-block') return; strut.style.setProperty('display', 'none', 'important'); }); });
    body.querySelectorAll('span:empty').forEach(function(strut) { if (isMeaningfulEmptySpan(strut) || getComputedStyle(strut).display !== 'inline-block') return; const rect = strut.getBoundingClientRect(); const block = vertical ? rect.width : rect.height; if (block > blockExtent + 1) strut.style.setProperty(vertical ? 'width' : 'height', Math.max(1, blockExtent) + 'px', 'important'); });
  };
  const prepareMedia = function() {
    const media = body.querySelectorAll('img, svg, image, video, canvas');
    for (let i = 0; i < media.length; i++) { const el = media[i]; el.style.breakInside = 'avoid'; el.style.pageBreakInside = 'avoid'; el.style.objectFit = 'contain'; if (verticalContext()) { el.style.maxInlineSize = '95vh'; el.style.maxBlockSize = '95vw'; } else { el.style.maxWidth = '95vw'; el.style.maxHeight = '95vh'; } if (el.tagName === 'IMG' && el.complete && (!el.naturalWidth || !el.naturalHeight)) { const alt = (el.getAttribute('alt') || '').trim(); if (alt && el.parentNode) { const fallback = document.createElement('span'); fallback.className = 'medicalreader-gaiji-fallback'; fallback.textContent = alt; el.parentNode.replaceChild(fallback, el); } } }
  };
  const waitForImages = function() {
    const pending = Array.from(body.querySelectorAll('img')).filter(function(image) { return !image.complete; });
    if (!pending.length) return Promise.resolve();
    return new Promise(function(resolve) { let remaining = pending.length, finished = false; const done = function() { if (!finished && remaining <= 0) { finished = true; setTimeout(resolve, 40); } }; pending.forEach(function(image) { const settle = function() { remaining -= 1; done(); }; image.addEventListener('load', settle, {once: true}); image.addEventListener('error', settle, {once: true}); }); setTimeout(function() { if (!finished) { finished = true; resolve(); } }, 1200); });
  };
  ...
  reader.calculateProgress = function() {
    const metrics = this.metrics || this.buildPaginationMetrics(); const stops = metrics.progressStops || [];
    if (!metrics.totalChars || !stops.length) return originalCalculateProgress();
    const current = position(); let low = 0, high = stops.length - 1, best = 0;
    while (low <= high) { const mid = Math.floor((low + high) / 2); if (stops[mid].scroll <= current + 1) { best = mid; low = mid + 1; } else high = mid - 1; }
  ...
  reader.restoreProgress = function(progress) {
    const target = this._pendingRestoreProgress == null ? progress : this._pendingRestoreProgress; this._pendingRestoreProgress = null;
    const metrics = this.metrics || this.buildPaginationMetrics();
    if (target <= 0) return this.setPagePosition(metrics.minScroll);
    if (target >= 0.99) return this.setPagePosition(metrics.maxScroll);
    const stops = metrics.progressStops || []; if (!stops.length) return originalRestoreProgress(target);
    const targetChars = Math.ceil(metrics.totalChars * target); let stop = stops[0];
  ...
  reader.buildPaginationMetrics = function() {
    sanitizeLayout(); prepareMedia();
    const metrics = originalBuildMetrics();
    if (!metrics || !metrics.progressStops) return metrics;
    const compact = []; let lastScroll = -1;
    for (let i = 0; i < metrics.progressStops.length; i++) { const stop = metrics.progressStops[i]; if (stop.scroll <= lastScroll + 0.5) continue; compact.push(stop); lastScroll = stop.scroll; }
    metrics.progressStops = compact; return metrics;
  };
  reader.contentLastPageScroll = function() { const metrics = this.metrics || this.buildPaginationMetrics(); return metrics ? metrics.maxScroll : maxScroll(); };

  const rebuild = function() {
    sanitizeLayout(); prepareMedia(); reader.metrics = null;
    const metrics = reader.buildPaginationMetrics();
    const target = reader._pendingRestoreProgress;
    reader._pendingRestoreProgress = null;
    if (target != null && reader.restoreProgress) reader.restoreProgress(target);
    else reader.setPagePosition(Math.min(metrics.maxScroll, reader.alignToPage(reader.position())));
  };

  sanitizeLayout(); prepareMedia();
  waitForImages().then(rebuild);
  window.addEventListener('resize', function() { reader._pendingRestoreProgress = reader.calculateProgress(); rebuild(); });
})();
''';
}
