...
(function() {
  ...
  const isPaginated = function() {
    return !!reader && reader.pageSize && reader.maxScroll && getComputedStyle(body).columnWidth !== 'auto';
  };
  ...
  const page = function(direction) {
    ...
    lastPagingAt = now;
    if (reader && reader.paginate) reader.paginate(direction);
  };

  window.medicalReaderPage = function(direction) {
    if (!isPaginated()) return 'ignored';
    page(direction === 'backward' ? 'backward' : 'forward');
    return 'ok';
  };
  ...
  focusStyle.textContent = '.medicalreader-focus { cursor: none; }.medicalreader-focus ::selection { background: rgba(120,120,120,.35); }';
  ...
  const emitSelection = function() {
    ...
    bridge({type: 'selection', selectedText: selectedText, sentence: sentenceFor(selectedText), href: location.href || '', startOffset: Number.isFinite(range.startOffset) ? range.startOffset : null, endOffset: Number.isFinite(range.endOffset) ? range.endOffset : null});
  };
