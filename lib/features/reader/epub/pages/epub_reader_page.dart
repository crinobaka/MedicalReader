...
class _EpubReaderPageState extends ConsumerState<EpubReaderPage> {
  ...
  void initState() {
    super.initState();
    _loadSettings();
    _controller = EpubReaderController(document: widget.document, initialPosition: ref.read(readerPositionStoreProvider).read(widget.document), onPositionSaved: _savePosition)..open().then((_) { if (mounted) setState(() {}); });
  }
  ...
  List<EpubNavItem> get _navigation {
    ...
    void visit(List<EpubNavItem> items, int depth) {
      for (final item in items) { result.add(EpubNavItem(title: '${List.filled(depth, '  ').join()}${item.title}', href: item.href, fragment: item.fragment, children: item.children)); visit(item.children, depth + 1); }
    }
  ...
  Future<void> _toggleBookmark() async {
    ...
    if (existing.isNotEmpty) { for (final item in existing) await notifier.remove(item.id); if (mounted) setState(() {}); return; }
  ...
  Future<void> _saveNote() async {
    ...
    final text = await showDialog<String>(context: context, builder: (context) => AlertDialog(title: Text(existing == null ? '添加笔记' : '编辑笔记'), content: TextField(controller: editor, autofocus: true, minLines: 4, maxLines: 10, decoration: const InputDecoration(hintText: '写下这一章的阅读笔记…')), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('取消')), FilledButton(onPressed: () => Navigator.pop(context, editor.text.trim()), child: const Text('保存'))]));
    editor.dispose();
  ...
  Future<void> _saveHighlight(ReaderLookupContext lookup) async {
    final href = lookup.href ?? _controller.book?.spine.elementAtOrNull(_controller.chapterIndex).idref;
    if (href == null || lookup.startOffset == null || lookup.endOffset == null) return;
    final notifier = ref.read(readerAnnotationsProvider(widget.document).notifier);
    final now = DateTime.now();
    final id = 'epub_highlight_${widget.document.id}_${href.hashCode}_${lookup.startOffset}_${lookup.endOffset}';
    await notifier.add(ReaderAnnotation(id: id, bookId: widget.document.id, pageIndex: _controller.chapterIndex, type: ReaderAnnotationType.highlight, content: lookup.selectedText, title: _chapterTitle(_controller.chapterIndex), locator: EpubReaderLocator(href: href, startOffset: lookup.startOffset, endOffset: lookup.endOffset, textQuote: lookup.textQuote ?? lookup.selectedText, prefix: lookup.prefix, suffix: lookup.suffix), createdAt: now, updatedAt: now));
    if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('已保存高亮')));
  }

  void _openAnnotations() {
    Navigator.of(context).pop();
    showModalBottomSheet<void>(context: context, isScrollControlled: true, builder: (_) => SafeArea(child: ListView(shrinkWrap: true, padding: const EdgeInsets.fromLTRB(16, 16, 16, 28), children: [Text('标注与笔记', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w700)), const SizedBox(height: 10), if (_annotations.isEmpty) const ListTile(title: Text('还没有保存的标注')), for (final item in _annotations) ListTile(leading: Icon(_iconForAnnotation(item.type)), title: Text(item.title.isNotEmpty ? item.title : _chapterTitle(item.pageIndex)), subtitle: Text(item.content.isNotEmpty ? item.content : item.type.name), onTap: () async { Navigator.of(context).pop(); await _controller.goToChapter(item.pageIndex.clamp(0, _controller.chapterCount - 1)); if (mounted) setState(() {}); })])));
  }

  IconData _iconForAnnotation(ReaderAnnotationType type) => switch (type) { ReaderAnnotationType.bookmark => Icons.bookmark_rounded, ReaderAnnotationType.note => Icons.sticky_note_2_rounded, ReaderAnnotationType.highlight => Icons.highlight_rounded, ReaderAnnotationType.tag => Icons.sell_outlined, ReaderAnnotationType.ink => Icons.draw_rounded };

  Future<void> _openLookup(ReaderLookupContext lookup) async {
    final highlighted = await showModalBottomSheet<bool>(context: context, isScrollControlled: true, builder: (_) => ReaderLookupSheet(contextData: lookup, dictionary: _dictionary, onHighlight: () async { Navigator.of(context).pop(true); await _saveHighlight(lookup); }));
    if (highlighted == true || !mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: Text(book.title, overflow: TextOverflow.ellipsis), actions: [IconButton(tooltip: _bookmarked ? '取消书签' : '书签', onPressed: _toggleBookmark, icon: Icon(_bookmarked ? Icons.bookmark : Icons.bookmark_border)), IconButton(tooltip: '阅读器面板', onPressed: _openSettings, icon: const Icon(Icons.tune_rounded))]), body: EpubReaderView(key: ValueKey('${_controller.chapterIndex}:${_controller.initialProgress}:${_controller.initialFragment}:${_settings.toJson()}'), archive: _controller.archive!, chapterIndex: _controller.chapterIndex, fragment: _controller.initialFragment, initialProgress: _controller.initialProgress, settings: _settings, annotations: _annotations, onPositionChanged: (href, progress) => _controller.updateProgress(href, progress), onPageBoundary: _handleBoundary, onSelectionChanged: _openLookup));
  }
