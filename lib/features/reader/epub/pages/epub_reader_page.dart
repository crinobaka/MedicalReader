...
class _EpubReaderPageState extends ConsumerState<EpubReaderPage> {
  ...
  Future<void> _saveHighlight(ReaderLookupContext lookup) async { final href = lookup.href ?? _controller.book?.spine.elementAtOrNull(_controller.chapterIndex).idref; if (href == null || lookup.startOffset == null || lookup.endOffset == null) return; final notifier = ref.read(readerAnnotationsProvider(widget.document).notifier); final now = DateTime.now(); final id = 'epub_highlight_${widget.document.id}_${href.hashCode}_${lookup.startOffset}_${lookup.endOffset}'; await notifier.add(ReaderAnnotation(id: id, bookId: widget.document.id, pageIndex: _controller.chapterIndex, type: ReaderAnnotationType.highlight, content: lookup.selectedText, title: _chapterTitle(_controller.chapterIndex), locator: EpubReaderLocator(href: href, startOffset: lookup.startOffset, endOffset: lookup.endOffset, textQuote: lookup.textQuote ?? lookup.selectedText, prefix: lookup.prefix, suffix: lookup.suffix), createdAt: now, updatedAt: now)); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('已保存高亮'))); }

  void _openAnnotations() { Navigator.of(context).pop(); showModalBottomSheet<void>(context: context, isScrollControlled: true, builder: (_) => SafeArea(child: ListView(shrinkWrap: true, padding: const EdgeInsets.fromLTRB(16, 16, 16, 28), children: [Text('标注与笔记', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w700)), const SizedBox(height: 10), if (_annotations.isEmpty) const ListTile(title: Text('还没有保存的标注')), for (final item in _annotations) ListTile(leading: Icon(_iconForAnnotation(item.type)), title: Text(item.title.isNotEmpty ? item.title : _chapterTitle(item.pageIndex)), subtitle: Text(item.content.isNotEmpty ? item.content : item.type.name), onTap: () async { Navigator.of(context).pop(); await _controller.goToChapter(item.pageIndex.clamp(0, _controller.chapterCount - 1)); if (mounted) setState(() {}); })]))); }
  ...
  Future<void> _openLookup(ReaderLookupContext lookup) async {
    final result = await showModalBottomSheet<Object?>(context: context, isScrollControlled: true, builder: (_) => ReaderLookupSheet(contextData: lookup, dictionary: _dictionary, onHighlight: () async { Navigator.of(context).pop(true); await _saveHighlight(lookup); }));
    if (result == true || result == null || !mounted) return;
    if (result is! DictionaryEntry) return;
    final mined = await showModalBottomSheet<bool>(context: context, isScrollControlled: true, builder: (_) => ReaderMiningSheet(entry: result, onMine: _mineToAnki));
