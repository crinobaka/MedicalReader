...
class BookTreeService {
  ...
  BookTreeService({
    ...
    PdfOutlineService outlineService = const PdfOutlineService(),
  ...
  Future<BookTreeIndex> loadIndexForDocument(
    ...
    return BookTreeIndex(
      nodes: _normalizeTree(nodes, pageCount: pageCount),
      pageCount: pageCount,
    );
  }
  ...
  Future<List<BookTreeNode>> loadForDocument(
    ...
    // The book-owned manifest is authoritative. Once materialized, the PDF
    // outline is never re-read, so user edits survive every restart.
    if (currentManifest != null) {
      if (currentManifest.bookTree.isNotEmpty) {
        return _parseNodeList(currentManifest.bookTree);
      }
      if (currentManifest.metadata['outlineImportAttempted'] == true) {
        return const [];
      }
    }

    // First open: import the embedded PDF outline and immediately turn it into
    // the same editable directory format used everywhere else in the app.
    ...
    if (currentManifest == null) {
      await _saveImportedTree(document, null, const []);
    }

    // Legacy compatibility for old installations.
  ...
  Map<String, dynamic> _nodeToJson(BookTreeNode node) => {
        ...
        if (node.children.isNotEmpty)
          'children': node.children.map(_nodeToJson).toList(growable: false),
      };
  ...
  bool _looksLikeNode(Map<String, dynamic> map) =>
      map.containsKey('id') || map.containsKey('name');
  ...
  List<BookTreeNode> _normalizeTree(
    ...
    for (var index = 0; index < nodes.length; index++) {
      ...
      normalized.add(BookTreeNode(
        ...
        children: _normalizeTree(node.children, pageCount: pageCount),
      ));
    }
